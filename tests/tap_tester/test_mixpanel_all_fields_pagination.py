    ...
    def pagination_test_run(self):
        ...
        for stream in streams_to_test_all_fields:
            with self.subTest(logging="Primary Functional Test", stream=stream):
                ...
                # These keys are not reliably produced by the Mixpanel API and
                # some fields are only present on a subset of records, so exact
                # equality across all messages is too strict for dynamic payloads.
                if stream == "export":
                ...
                # Engage contains dynamic fields that vary by record; keep the
                # strict all-fields assertion for the other streams.
                if stream != "engage":
